...
const SignUpScreen = ({ navigation }) => {
  ...
  const handleRegister = async() => {
    ...
    fetch(API_URL, {
        ...
        body: JSON.stringify({
          "query": "mutation customerCreate($input: CustomerCreateInput!) { customerCreate(input: $input) { customer { firstName lastName, email, acceptsMarketing } customerUserErrors { field, message, code } } }",
           "variables": {
              "input": {
                "firstName": `${firstName}`,
                "lastName": `${lastName}`,
